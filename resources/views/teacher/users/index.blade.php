...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <div>
            @can('create users')
            <a class="btn btn-success btn-sm me-2" href="{{ route('teacher.users.create') }}">
                <span data-feather="user-plus"></span> Add User
            </a>
            <a class="btn btn-primary btn-sm" href="{{ route('teacher.users.bulk-create') }}">
        ...
        <ul class="nav nav-tabs mb-3">
            <li class="nav-item">
                <a class="nav-link {{ $tab === 'students' ? 'active' : '' }}"
                    href="{{ route('teacher.users.index', ['tab' => 'students']) }}">
                    <span data-feather="users"></span> Learners
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link {{ $tab === 'teachers' ? 'active' : '' }}"
                    href="{{ route('teacher.users.index', ['tab' => 'teachers']) }}">
                    <span data-feather="user-check"></span> Educators
                </a>
            </li>
        </ul>
                    ...
                    <tr data-entry-id="{{ $user->id }}">
                        ...
                        <td>
                            <div class="btn-group" role="group">
                                @can('edit users')
                                <a href="{{ route('teacher.users.edit', $user->id) }}"
                                ...
                                @can('delete users')
                                @if (auth()->user()->hasRole('Educator') && $user->id !== auth()->id())
                                @if($user->status)
                                <form action="{{ route('teacher.user.toggleStatus', ['id' => $user->id]) }}" method="POST"
...
<div class="alert alert-info mt-4">
    ...
    <ul class="mb-0 mt-2">
        ...
        <li>Use the <strong>tabs</strong> (Learners, Educators) to filter users by role</li>
    </ul>
</div>
