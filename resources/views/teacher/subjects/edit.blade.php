    ...
    <div style="background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); border-radius: 1rem; padding: 2rem; margin-bottom: 2rem; color: white; box-shadow: 0 10px 25px rgba(99, 102, 241, 0.2);">
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
            <div>
                ...
                <p style="margin: 0; opacity: 0.95; font-size: 1rem;">Update course details and settings</p>
            </div>
        ...
        <form action="{{ route('teacher.subjects.update', $subject->id) }}" method="POST">
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <input type="text" 
                       id="name" 
                       name="name" 
                       value="{{ old('name', $subject->name ?? $subject->subjects_name) }}" 
                       required
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <select id="code" 
                    ...
                    <option value="Year 1" {{ old('code', $subject->code ?? $subject->subjects_code) === 'Year 1' ? 'selected' : '' }}>Year 1</option>
                    <option value="Year 2" {{ old('code', $subject->code ?? $subject->subjects_code) === 'Year 2' ? 'selected' : '' }}>Year 2</option>
                    <option value="Year 3" {{ old('code', $subject->code ?? $subject->subjects_code) === 'Year 3' ? 'selected' : '' }}>Year 3</option>
                    <option value="Form 1" {{ old('code', $subject->code ?? $subject->subjects_code) === 'Form 1' ? 'selected' : '' }}>Form 1</option>
                    <option value="Form 2" {{ old('code', $subject->code ?? $subject->subjects_code) === 'Form 2' ? 'selected' : '' }}>Form 2</option>
                    <option value="Form 3" {{ old('code', $subject->code ?? $subject->subjects_code) === 'Form 3' ? 'selected' : '' }}>Form 3</option>
                    <option value="Form 4" {{ old('code', $subject->code ?? $subject->subjects_code) === 'Form 4' ? 'selected' : '' }}>Form 4</option>
                    <option value="Form 5" {{ old('code', $subject->code ?? $subject->subjects_code) === 'Form 5' ? 'selected' : '' }}>Form 5</option>
                </select>
                @error('code')
                <div style="color: #ef4444; font-size: 0.875rem; margin-top: 0.5rem; display: flex; align-items: center; gap: 0.5rem;">
                    <i data-feather="alert-circle" style="width: 16px; height: 16px;"></i>
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <textarea id="description" 
                          ...
                          onfocus="this.style.borderColor='#6366f1'; this.style.background='white';"
                          onblur="this.style.borderColor='#e2e8f0'; this.style.background='#f8fafc';">{{ old('description', $subject->description) }}</textarea>
                @error('description')
                <div style="color: #ef4444; font-size: 0.875rem; margin-top: 0.5rem; display: flex; align-items: center; gap: 0.5rem;">
                    <i data-feather="alert-circle" style="width: 16px; height: 16px;"></i>
                    {{ $message }}
            ...
            <div style="background: #f8fafc; border-radius: 0.75rem; padding: 1.5rem; margin-bottom: 2rem; border: 1px solid #e2e8f0;">
                ...
                <div style="display: flex; align-items: center; gap: 0.75rem; padding: 1rem; background: white; border-radius: 0.5rem; border: 1px solid #e2e8f0; transition: all 0.3s;"
                    ...
                    <input type="checkbox" 
                           ...
                           name="is_published" 
                           value="1" 
                           {{ old('is_published', $subject->is_published ?? false) ? 'checked' : '' }}
                           style="width: 20px; height: 20px; cursor: pointer; accent-color: #6366f1;">
            ...
            <div style="display: flex; gap: 1rem; justify-content: flex-end; padding-top: 1.5rem; border-top: 1px solid #e2e8f0;">
                ...
                <button type="submit" 
                        style="padding: 0.875rem 2rem; background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); color: white; border: none; border-radius: 0.5rem; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 0.5rem; transition: all 0.3s; box-shadow: 0 4px 6px rgba(99, 102, 241, 0.3);"
