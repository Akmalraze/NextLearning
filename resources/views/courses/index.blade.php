...
<div class="container py-5" style="max-width: 1400px;">
    <div class="text-center mb-5">
        <h1 class="mb-2" style="font-weight: 800; color: #0f172a; font-size: 2.5rem;">Browse Courses</h1>
        <p class="text-muted mb-0" style="font-size: 1.1rem;">Explore available courses and join the ones that interest you</p>
    </div>
    ...
    @if($courses->isEmpty())
        <div class="text-center py-5">
            <div style="background: white; border-radius: 1rem; padding: 3rem; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                ...
                <h3 style="color: #1e293b; margin-bottom: 1rem;">No courses are published yet</h3>
                <p class="text-muted mb-0">Please check back later for available courses.</p>
            </div>
        </div>
    @else
        <div class="row g-4">
            @foreach($courses as $course)
                @php
                    ...
                    if ($user && $user->hasRole('Learner')) {
                        $isEnrolled = $course->learners()->where('users.id', $user->id)->exists();
                    }
                @endphp
                <div class="col-lg-4 col-md-6">
                    <div style="background: white; border-radius: 1rem; padding: 1.5rem; box-shadow: 0 4px 6px rgba(0,0,0,0.1); height: 100%; display: flex; flex-direction: column; transition: transform 0.3s, box-shadow 0.3s; border-left: 4px solid #6366f1;" 
                        ...
                        <div style="margin-bottom: 1rem;">
                            ...
                            <div style="display: flex; gap: 0.5rem; flex-wrap: wrap; margin-bottom: 0.75rem;">
                                <span style="background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); color: white; padding: 0.375rem 0.75rem; border-radius: 0.5rem; font-size: 0.875rem; font-weight: 600;">
                                    {{ $course->code }}
                                </span>
                        ...
                        <div style="background: #f8fafc; border-radius: 0.75rem; padding: 1rem; margin-bottom: 1.5rem;">
                            ...
                            @if($course->educator)
                                <div style="display: flex; align-items: center; gap: 0.75rem;">
                                    <div style="width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); display: flex; align-items: center; justify-content: center; font-weight: 700; color: white; font-size: 1.1rem; flex-shrink: 0;">
                        ...
                        <div style="display: flex; gap: 0.75rem;">
                            @auth
                                @if(auth()->user()->hasRole('Learner'))
                                    @if($isEnrolled)
                                        <div style="flex: 1; background: #10b981; color: white; padding: 0.75rem 1rem; border-radius: 0.5rem; text-align: center; font-weight: 600; font-size: 0.9rem; display: flex; align-items: center; justify-content: center; gap: 0.5rem;">
                                            <i data-feather="check-circle" style="width: 18px; height: 18px;"></i>
                                            Enrolled
                                        </div>
                                    @else
                                        <form method="POST" action="{{ route('courses.enroll', $course) }}" style="flex: 1; margin: 0;">
                                            @csrf
                                            <button type="submit" style="width: 100%; background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; padding: 0.75rem 1rem; border-radius: 0.5rem; border: none; font-weight: 600; font-size: 0.9rem; cursor: pointer; transition: transform 0.2s; display: flex; align-items: center; justify-content: center; gap: 0.5rem;" 
                            ...
                            @endauth
                            <a href="{{ route('courses.show', $course) }}" style="background: #f1f5f9; color: #475569; padding: 0.75rem 1rem; border-radius: 0.5rem; text-align: center; font-weight: 600; font-size: 0.9rem; text-decoration: none; display: flex; align-items: center; justify-content: center; gap: 0.5rem; transition: all 0.2s; border: 1px solid #e2e8f0;" 
